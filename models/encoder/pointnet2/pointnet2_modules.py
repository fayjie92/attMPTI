...
if 0:
  ...
  import models.encoder.pointnet2.pointnet2_utils as pt2_utils
...
class PointnetSAModuleVotes(nn.Module):
  ...
  def __init__(
    ...
    if npoint is not None:
      self.grouper = pt2_utils.QueryAndGroup(radius, nsample,
        use_xyz=use_xyz, ret_grouped_xyz=True, normalize_xyz=normalize_xyz,
        sample_uniformly=sample_uniformly, ret_unique_cnt=ret_unique_cnt)
    else:
      self.grouper = pt2_utils.GroupAll(use_xyz, ret_grouped_xyz=True)
  ...
  def forward(self, xyz: torch.Tensor,
    ...
    if self.npoint is not None:
      inds = pt2_utils.furthest_point_sample(xyz, self.npoint)
    #else:
    #    assert(inds.shape[1] == self.npoint)
    new_xyz = pt2_utils.gather_operation(
      xyz_flipped, inds
    ).transpose(1, 2).contiguous() if self.npoint is not None else None
...
class PointnetFPModule(nn.Module):
  ...
  def forward(
    ...
    if known is not None:
      ...
      interpolated_feats = pt2_utils.three_interpolate(
